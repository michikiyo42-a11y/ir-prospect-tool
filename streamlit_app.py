...
def find_annual_report(company_name: str, log) -> Optional[dict]:
    today = datetime.now()
        ...
        try:
            ...
            for doc in resp.json().get("results", []):
                if (company_name in doc.get("filerName", "")
                    ...
                    log(f"発見: {doc['filerName']} ({date_str})")
                    return doc
        except Exception:
            pass
        time.sleep(0.15)
...
def analyze(company_name: str, text: str, doc_info: Optional[dict]) -> dict:
    ...
    return {
        "企業名":                  company_name,
        "正式名称":                filer,
        "業種":                    doc_info.get("industryName", "不明") if doc_info else "不明",
...
with col2:
    ...
    if companies:
        for c in companies:
            st.write(f"・{c}")
        st.info(f"合計 {len(companies)} 社　推定時間: 約 {len(companies) * 2} 〜 {len(companies) * 3} 分")
    else:
        st.write("企業名を入力すると、ここにリストが表示されます")

st.divider()

if st.button("🚀 分析スタート", type="primary", disabled=not companies):
    results = []
    ...
    for i, company in enumerate(companies):
        ...
        log(f"EDINETを検索しています...")

        doc_info = find_annual_report(company, log)

        if doc_info:
            log(f"文書をダウンロード中...")
            text = download_text(doc_info["docID"])
            log(f"テキスト取得: {len(text):,}文字")
        else:
            log(f"EDINETで見つかりませんでした")
            text = ""

        result = analyze(company, text, doc_info)
